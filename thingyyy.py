...
pygame.display.set_caption("Clicker increments 0.γ")
...
def format(a):
	if a < 10 ** 6:
		...
		return str(math.floor((a / (10 ** (math.floor(len(str(int(a))) - 1)))) * (10 ** per)) / (10 ** per)) + "e" + str(math.floor(len(str(int(a)))) - 1)
...
def update_auto_income():
	...
	if time_passed >= 1000 / FPS:  # Каждую секунду
		...
		click_count += (click_multiplier * frames * auto_income * (RDollars + 1)) ** click_exponent / FPS
...
rebirth_btn = Button(
	230, 20, UPGRADE_BTN_WIDTH, UPGRADE_BTN_HEIGHT,
	"Rebirth for\n0 RP",
...
def draw_stats():
	...
	hint2 = small_font.render(f"FPS: {FPS}, Precision: {per}", True, TEXT_COLOR)
	screen.blit(hint2, (circle_x - hint2.get_width() // 2, circle_y + CIRCLE_RADIUS + 230))

	hint3 = small_font.render("v0.γ", True, TEXT_COLOR)
	screen.blit(hint3, (circle_x - hint3.get_width() // 2, circle_y + CIRCLE_RADIUS - 410))

	RP = small_font.render(f"R$: {format(RDollars)}, x{format(RDollars + 1)}", True, (100, 100, 200))
	screen.blit(RP, (circle_x - RP.get_width() // 2, circle_y + CIRCLE_RADIUS - 390))
...
def update_upgrade_buttons():
	...
	if click_count > 1000000:
		rebirth_btn.text = f"Rebirth for \n {format(math.cbrt(click_count / 1000000 ) - 1)}R$"
	else:
		rebirth_btn.text = "Rebirth unlocks at 1.0e6$"
...
def check_click(mouse_pos):
	...
	if distance < CIRCLE_RADIUS:
		click_count += (click_multiplier * (RDollars + 1)) ** click_exponent
		play_click_sound()
		return True
	return False
...
while running:
		...
		if event.type == pygame.QUIT:
			...
			if event.button == 1:
				if click_upgrade_btn.is_clicked(mouse_pos):
					...
					per = change_per()
				elif rebirth_btn.is_clicked(mouse_pos):
					if click_count > 1000000:
						RDollars = RDollars + math.cbrt(click_count / 1000000 ) - 1
						click_count = 0
						click_multiplier = 1
						click_exponent = 1
						auto_income = 0
	...
	rebirth_btn.draw(screen, small_font, mouse_pos)
